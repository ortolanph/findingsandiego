...
public class LinearSearchRefactoredSandbox {
    ...
    public static void main(String[] args) throws Exception{
        ...
        report.report(String.format("LINEAR SEARCH REFACTORED - By Gender: [%s]", Gender.FEMALE), results1);

        List<Burglar> results2 = search.find(Gender.FEMALE, Hair.BROWN, null, null, null);

        report.report(String.format("LINEAR SEARCH REFACTORED - By Gender and Hair: [%s, %s]", Gender.FEMALE, Hair.BROWN), results2);

        List<Burglar> results3 = search.find(Gender.FEMALE, Hair.BROWN, Eye.BROWN, null, null);

        report.report(String.format("LINEAR SEARCH REFACTORED - By Gender, Hair and Eye: [%s, %s, %s]", Gender.FEMALE, Hair.BROWN, Eye.BROWN), results3);

        List<Burglar> results4 = search.find(Gender.FEMALE, Hair.BROWN, Eye.BROWN, BURGLAR_OCCUPATION, null);

        report.report(String.format("LINEAR SEARCH REFACTORED - By Gender, Hair, Eye and Occupation: [%s, %s, %s, %s]", Gender.FEMALE, Hair.BROWN, Eye.BROWN, BURGLAR_OCCUPATION), results4);

        List<Burglar> results5 = search.find(Gender.FEMALE, Hair.BROWN, Eye.BROWN, BURGLAR_OCCUPATION, MEXICAN_FOOD);

        report.report(String.format("LINEAR SEARCH REFACTORED - By Gender, Hair, Eye, Occupation and Favorite Food: [%s, %s, %s, %s, %s]", Gender.FEMALE, Hair.BROWN, Eye.BROWN, BURGLAR_OCCUPATION, MEXICAN_FOOD), results5);
    }
}
